...
def agent_action(
    account_id: str, payload: AgentIn, ctx: AuthContext = Depends(get_context), db: Session = Depends(get_db)
):
    account = _account(db, ctx, account_id)
    assert_can_run_ai(ctx.organization)
    run = run_agent(
...
async def upload_document(
    ...
    if name.endswith((".txt", ".md", ".csv", ".json")):
        ...
        text = f"Uploaded binary file {file.filename} ({file.content_type}). Text extraction is limited to txt/md/csv/json locally."
    doc = Document(
        id=uid(),
        org_id=ctx.organization.id,
        account_id=account.id,
